...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        next_obs_pred = delta_pred_normalized * delta_std + delta_mean + obs_unnormalized # TODO(Q1)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction = self(obs, acs, **{key: ptu.from_numpy(value) for key, value in data_statistics.items()}) # TODO(Q1) get numpy array of the predicted next-states (s_t+1)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        prediction = ptu.to_numpy(prediction[0])
        return prediction
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        prediction = self(obs, acs, **{key: ptu.from_numpy(value) for key, value in data_statistics.items()})[1]

        loss = nn.MSELoss()(prediction, target) # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
