...
class PGAgent(BaseAgent):
    ...
    def _discounted_return(self, rewards):
        ...
        discount_factors = np.array([self.gamma ** n for n in range(len(rewards))])
        discounted_return = np.sum(discount_factors * rewards)
        list_of_discounted_returns = [discounted_return for _ in rewards]
    ...
    def _discounted_cumsum(self, rewards):
        ...
        discount_factors_at_zero = [self.gamma ** n for n in range(len(rewards))]
        total_T = len(rewards)
        discount_factors = np.array([[0] * n + discount_factors_at_zero[: (total_T - n)] for n in range(total_T)])
        list_of_discounted_cumsums = np.matmul(discount_factors, rewards)
